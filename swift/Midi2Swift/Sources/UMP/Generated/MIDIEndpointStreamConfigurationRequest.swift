...
/// MIDIEndpoint.StreamConfigurationRequest (UMP128)
/// Source: libs/messageTypes.js:68
public struct MIDIEndpointStreamConfigurationRequest: Equatable {
    ...
    public func encode() -> UMP128 {
        ...
        (lo, hi) = setBits128(lo, hi, 5, offset: 6, width: 10)
        (lo, hi) = setBits128(lo, hi, UInt64(_protocol), offset: 16, width: 8)
        return UMP128(lo: lo, hi: hi)
    }

    public static func decode(_ ump: UMP128) -> MIDIEndpointStreamConfigurationRequest? {
        if getBits128(ump.lo, ump.hi, offset: 0, width: 4) != 15 { return nil }
        if getBits128(ump.lo, ump.hi, offset: 6, width: 10) != 5 { return nil }
        let form = UInt8(getBits128(ump.lo, ump.hi, offset: 4, width: 2))
        let _protocol = UInt8(getBits128(ump.lo, ump.hi, offset: 16, width: 8))
        return MIDIEndpointStreamConfigurationRequest(form: form, _protocol: _protocol)
    }
}
