...
/// FlexDataMessages.FlexDataMessages0x2FlexDataMessages0x1 (UMP128)
/// Source: libs/messageTypes.js:68
public struct FlexDataMessagesFlexDataMessages0x2FlexDataMessages0x1: Equatable {
    ...
    public func encode() -> UMP128 {
        ...
        { let tmp = setBits128(lo, hi, 2, offset: 16, width: 8); lo = tmp.0; hi = tmp.1 }
        { let tmp = setBits128(lo, hi, 1, offset: 24, width: 8); lo = tmp.0; hi = tmp.1 }
        return UMP128(lo: lo, hi: hi)
    }

    public static func decode(_ ump: UMP128) -> FlexDataMessagesFlexDataMessages0x2FlexDataMessages0x1? {
        if getBits128(ump.lo, ump.hi, offset: 0, width: 4) != 13 { return nil }
        if getBits128(ump.lo, ump.hi, offset: 16, width: 8) != 2 { return nil }
        if getBits128(ump.lo, ump.hi, offset: 24, width: 8) != 1 { return nil }
        ...
        return FlexDataMessagesFlexDataMessages0x2FlexDataMessages0x1(channel: channel, group: group, form: form)
    }
}
